...
class ParameterSnapshot(Base):
    ...
    ema_fast: Mapped[int] = mapped_column(Integer, default=10)
    ema_slow: Mapped[int] = mapped_column(Integer, default=50)
    volume_multiplier: Mapped[float] = mapped_column(Float, default=1.3)
    min_signal_strength: Mapped[float] = mapped_column(Float, default=1.5)
    ...
    # Exit parameters
    atr_stop_multiplier: Mapped[float] = mapped_column(Float, default=2.5)
    atr_target_multiplier: Mapped[float] = mapped_column(Float, default=4.0)
    default_stop_loss_pct: Mapped[float] = mapped_column(Float, default=5.0)
    default_profit_target_pct: Mapped[float] = mapped_column(Float, default=10.0)
    max_hold_days: Mapped[int] = mapped_column(Integer, default=25)
